...
-- ---------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Question:2
-- Write a query to display (product_class_desc, product_id, product_desc, product_quantity_avail ) 
...
-- For Electronics and Computer categories  
    when P.PRODUCT_CLASS_CODE in (2050, 2053) then 
		case  
		    when P.PRODUCT_QUANTITY_AVAIL = 0 then 'Out_of_stock'
	        when P.PRODUCT_QUANTITY_AVAIL <= 10 then 'Low_stock'
		    when (P.PRODUCT_QUANTITY_AVAIL >= 11 and P.PRODUCT_QUANTITY_AVAIL <= 30) then 'In_stock'
            when P.PRODUCT_QUANTITY_AVAIL >= 31 then 'Enough_Stock'
		End
...
-- For Stationery and Clothes categories
    when P.PRODUCT_CLASS_CODE in ( 2052, 2056) then 
		case  
		    when P.PRODUCT_QUANTITY_AVAIL = 0 then 'Out_of_stock'
	        when P.PRODUCT_QUANTITY_AVAIL <= 20 then 'Low_stock'
		    when (P.PRODUCT_QUANTITY_AVAIL >= 21 and P.PRODUCT_QUANTITY_AVAIL <= 80) then 'In_stock'
            when P.PRODUCT_QUANTITY_AVAIL >= 81 then 'Enough_Stock'
		End 
...
-- For rest of the categories
		...
		case  
		    when P.PRODUCT_QUANTITY_AVAIL = 0 then 'Out_of_stock'
	        when P.PRODUCT_QUANTITY_AVAIL <= 15 then 'Low_stock'
		    when (P.PRODUCT_QUANTITY_AVAIL >= 16 and P.PRODUCT_QUANTITY_AVAIL <= 50) then 'In_stock'
            when P.PRODUCT_QUANTITY_AVAIL >= 51 then 'Enough_Stock'
        End  
...
-- ---------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Question:3
-- Write a query to Show the count of cities in all countries other than USA & MALAYSIA, with more than 1 city, in the descending order of CITIES.
-- (2 rows)[NOTE :ADDRESS TABLE]

select count(CITY) as COUNT_OF_CITIES, COUNTRY 
from ADDRESS
group by COUNTRY
...
-- ---------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Question:4
-- Write a query to display the customer_id,customer full name ,city,pincode,and order details (order id, product class desc, product desc, 
-- subtotal(product_quantity * product_price)) for orders shipped to cities whose pin codes do not have any 0s in them.
...
-- ---------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Question:5
-- Write a Query to display product id,product description,totalquantity(sum(product quantity) for a
...
select OC.CUSTOMER_ID, concat (OC.CUSTOMER_FNAME,' ',OC.CUSTOMER_LNAME) as CUSTOMER_FULL_NAME, OH.ORDER_ID, OI.PRODUCT_QUANTITY,
...
-- ---------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Question:9
-- Write a query to display the order_id,customer_id and customer fullname starting with “A” along
-- with (product quantity) as total quantity of products shipped for order ids > 10030 (5 Rows) 
-- [NOTE: TABLES to be used-online_customer,Order_header, order_items] 

select OH.ORDER_ID, OC.CUSTOMER_ID, concat (OC.CUSTOMER_FNAME,' ',OC.CUSTOMER_LNAME) as CUSTOMER_FULL_NAME,
sum(OI.PRODUCT_QUANTITY) as TOTAL_PRODUCT_QTY 
from ONLINE_CUSTOMER OC 
           inner join ORDER_HEADER OH on OC.CUSTOMER_ID = OH.CUSTOMER_ID
		   inner join ORDER_ITEMS OI on OH.ORDER_ID = OI.ORDER_ID
...
-- ---------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Question:10
-- Write a query to display product class description, totalquantity(sum(product_quantity), 
-- Totalvalue (product_quantity * product price) and show which class of products have been shipped
-- highest(Quantity) to countries outside India other than USA? Also show the total value of those items.(1 ROWS)
